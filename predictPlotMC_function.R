...
predict.plot.mc <- function (x, VariableName = NULL, VarPlot = "Both", color.palette = rainbow(nclasses, 
                                                                            start = 0.66, end = 0.99), nclasses = 10, breaktype = "quantile", 
          dec.dig = 2, SEcex.min = 0.5, SEcex.max = 2, brks = NULL, 
          add = FALSE, ...) 
{
  par.orig <- par(no.readonly = TRUE)
  if (class(x) != "glmssn.predict") 
    return("Not a glmssn.predict object")
  ...
  if (add == FALSE) {
    ...
    par(mar = c(5, 5, 3, 0))
    plot(c(-2200000,-2150000), c(2600000,2800000), 
         type = "n", xlab = "x-coordinate", ylab = "y-coordinate", 
         main = paste("Prediction Variable = ", zcol, ":  Plotting", 
                      VarPlot), cex.main = 0.9, ...)
    for (i in 1:length(x$ssn.object@lines)) for (j in 1:length(x$ssn.object@lines[[i]])) lines((x$ssn.object@lines[[i]]@Lines[[j]]@coords), 
                                                                                               ...)
  }
  predpointsID <- x$args$predpointsID
  for (i in 1:length((x$ssn.object@predpoints@SSNPoints))) if (x$ssn.object@predpoints@ID[i] == 
                                                                 predpointsID) {
  ...
  if (breaktype == "quantile") {
    brks <- quantile(datap[, zcolplot], probs = (1:(nclasses - 
                                                      1))/nclasses, na.rm = T)
    lower.breaks <- c(min(datap[, zcolplot], na.rm = T), 
                      brks)
    upper.breaks <- c(brks, max(datap[, zcolplot], na.rm = T))
  }
  if (breaktype == "even") {
    brks <- min(datap[, zcolplot]) + (max(datap[, zcolplot]) - 
                                        min(datap[, zcolplot])) * (1:(nclasses - 1))/nclasses
    lower.breaks <- c(min(datap[, zcolplot], na.rm = T), 
                      brks)
    upper.breaks <- c(brks, max(datap[, zcolplot], na.rm = T))
  }
  if (breaktype == "user") {
    ...
    if (nclasses > length(color.palette)) {
      ...
      color.palette <- c(color.palette, sample(newColours, 
                                               nclasses - length(color.palette)))
    }
    if (nclasses < length(color.palette)) {
  ...
  if (VarPlot == "Both") 
    SErange <- max(datap[, zSEcol]) - min(datap[, zSEcol])
  if (add == TRUE) {
    par(new = TRUE)
    layout(matrix(1:2, nrow = 1), widths = c(4, 1))
    par(mar = c(5, 5, 3, 0))
    par(mfg = c(1, 1))
    plot(x$ssn.object@bbox[1, ], x$ssn.object@bbox[2, ], 
         type = "n", bty = "n", xlab = "", ylab = "", ...)
  }
  for (j in 1:nclasses) {
    jmax <- upper.breaks[j]
    jmin <- lower.breaks[j]
    indj <- datap[, zcolplot] >= jmin & datap[, zcolplot] <= 
      jmax
    if (VarPlot == "Both") {
      points(pcoord[indj, ], col = color.palette[j], pch = 19, 
             cex = SEcex.max - (SEcex.max - SEcex.min) * (datap[indj, 
                                                                zSEcol] - min(datap[, zSEcol]))/SErange, ...)
    }
  ...
  if (add == FALSE) {
    ...
    if (VarPlot == "Both") 
      title = "Predictions"
    legend("bottomleft", legend = leglabs, bty = "n", pch = rep(plch, 
                                                                times = length(leglabs)), col = color.palette, title = title, 
           cex = 0.8)
    if (VarPlot == "Both") {
      title = "Standard Errors"
      cexVals <- SEcex.max - (SEcex.max - SEcex.min) * 
        (datap[, zSEcol] - min(datap[, zSEcol]))/SErange
      cexVals <- quantile(cexVals, seq(0, 1, by = 0.2))
      cexLab <- signif(quantile(datap[, zSEcol], seq(1, 
                                                     0, by = -0.2)), 3)
      legend("topleft", legend = cexLab, bty = "n", pch = 1, 
             pt.cex = cexVals, col = 1, title = title, cex = 0.8)
    }
    par(par.orig)
  ...
  if (add == TRUE) {
    ...
    plot(c(0, 0), c(1, 1), type = "n", xaxt = "n", yaxt = "n", 
         xlab = "", ylab = "", bty = "n")
    lp <- as.character(as.numeric(as.integer(min(datap[, 
                                                       zcolplot]) * 10^dec.dig))/10^dec.dig)
    up <- as.character(as.numeric(as.integer(max(datap[, 
                                                       zcolplot]) * 10^dec.dig))/10^dec.dig)
    text(0, 1.3, paste("lowest pred =", lp), cex = 0.8)
    text(0, 0.7, paste("highest pred =", up), cex = 0.8)
    par(par.orig)
  }
}