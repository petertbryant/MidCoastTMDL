subsetSSN_noedges <- function (ssn, filename, subset, clip = FALSE) 
{
  dir.create(filename)
  oldwd <- getwd()
  setwd(filename)
...
#     edges.sub <- SpatialLinesDataFrame(edges.sl, ssn.tmp@data[ind.edges, 
#                                                               ], match.ID = FALSE)
...
#     rm(proj4string, sl, edges.sl, edges.sub, ind.edges)
    ...
    if (pred.len > 0) {
      for (i in 1:pred.len) {
        ...
        ind.xy <- names(data.tmp) == "coords_x1" | names(data.tmp) == 
          "coords_x2"
        if (sum(ind.xy) > 0) {
          data.tmp <- data.tmp[, !ind.xy]
        }
        preds.sub <- SpatialPointsDataFrame(coords = coords, 
                                            data = data.tmp, proj4string = proj4string)
        writeSpatialShape(preds.sub, pred.name)
        SSN:::write.dbf.SSN(preds.sub@data, pred.name, max_nchar = 30)
        rm(coords, proj4string, data.tmp, preds.sub, 
           ind.preds, ind.xy)
      }
    }
#     for (i in 1:length(netID.list)) {
#       fn.old <- file.path(ssn@path, paste("netID", netID.list[i], 
#                                           ".dat", sep = ""))
#       fn.new <- file.path(ssn.tmp@path, paste("netID", 
#                                               netID.list[i], ".dat", sep = ""))
#       file.copy(fn.old, fn.new, overwrite = TRUE)
#     }
#     rm(fn.new, fn.old)
  }
  ...
  sites.sub <- SpatialPointsDataFrame(coords = coords, data = data.tmp, 
  ...
  if (pred.len > 0) {
    ...
    ssn.tmp <- importSSN(ssn.tmp@path, o.write = T)
    for (j in 1:pred.len) {
      ssn.tmp <- importPredpts(ssn.tmp, pred.name.list[j], 
                               obj.type = "ssn")
    }
  }
  setwd(oldwd)
