...
fss2.s2.rm.rf.vi.l <- melt(fss2.s2.rm.rf.vi)

fss2.s2.rm.rf.vi.median <- cast(fss2.s2.rm.rf.vi.l,variable ~ ., value ='value', median)
colnames(fss2.s2.rm.rf.vi.median )[2] <- "median"

# sort the data so largest at the top
fss2.s2.rm.rf.vi.median <- fss2.s2.rm.rf.vi.median[with(fss2.s2.rm.rf.vi.median, order(-median)), ]
fss2.s2.rm.rf.vi.median$variable <- as.character(fss2.s2.rm.rf.vi.median$variable)
##############################

#Partial dependence plots
for (i in 1:length(fss2.s2.rm.rf.vi.median$variable)) {
  filename <- paste("partialPlot_", fss2.s2.rm.rf.vi.median$variable[i], "_FirstRun.png",sep = "")
  png(filename, width = 960, height = 960)
  partialPlot(fss2.s2.rm.rf, 
              fss2.s2, 
              x.var = fss2.s2.rm.rf.vi.median$variable[i], 
              ylab = 'Mean FSS', 
              ylim = c(9,18))
  dev.off()  
}
